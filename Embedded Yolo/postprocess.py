...
from boxtargets import BoxTarget, boxlist_nms, remove_small_box, cat_boxlist
...
class FCOSPostprocessor(nn.Module):
    ...
    def forward_single_feature_map(self, location, cls_pred, box_pred, center_pred, image_sizes):
        ...
        for i in range(batch):
            ...
            top_n = top_ns[i]
            if candid_id.sum().item() > top_n.item():  # falls Kandidaten Anzahl > self.top_n (1000)
                # top_n ist immer == candid_id.sum() außer, wenn > 1000 pot Kandidaten
    ...
    def select_over_scales(self, boxlists):
        ...
        for boxlist in boxlists:
            ...
            for j in range(1, self.n_class):
                ...
                box_by_class = BoxTarget(box_j, boxlist.size, mode='xyxy')
